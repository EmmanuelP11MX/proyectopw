    ...
    if (localStorage.getItem('listaDeseo') == null) {
        ...
        for (let i = 0; i < listaExiste.length; i++) {
            if (listaExiste[i]['idProducto'] == idProducto) {
                Swal.fire(
                    'Aviso',
                    'Este producto ya lo tienes en tu lista de deseos',
                    'warning'
                );
                return;
            }   
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            res.productos.forEach(producto => {
                ...
                <tr>
                    ...
                    <td>
                        <button class="btn btn-danger btnDeleteCart" type="button" prod="${producto.id_producto}"><i class="fas fa-times-circle"></i></button>
                    </td>
                </tr>`;
            });
            tableListaCarrito.innerHTML = html;
            document.querySelector('#totalGeneral').textContent = res.subtotal;
            btnEliminarCarrito();
        }
    }
}
